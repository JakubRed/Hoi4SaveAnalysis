...
# Connect to database
def run_data_extraction(json_path):
    ...
    for i in range(1):
        ...
        export_general_info_to_sql(cursor, json_path, dataset_id)
        export_construction_to_sql(cursor, json_path, tracked_countries, dataset_id)
        export_countries_to_sql(cursor, json_path, tracked_countries, dataset_id)
        export_dataset_date(cursor, json_path, dataset_id)
        export_equipment_production_to_sql(cursor, json_path, tracked_countries, dataset_id)
        export_fuel_to_sql(cursor, json_path, tracked_countries, dataset_id)
        export_states_to_sql(cursor, json_path, tracked_countries, dataset_id)
